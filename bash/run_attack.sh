...
# [SBATCH 옵션]
# ==========================================
#SBATCH --job-name=MANO_pgd      # 작업 이름
#SBATCH --partition=RTX3090
#SBATCH --nodes=1                   # 노드 개수
#SBATCH --gres=gpu:3                # [중요] 사용 GPU 인덱스가 0~6이므로 8개 요청 (혹은 7개)
#SBATCH --cpus-per-task=16          # CPU 코어 수
#SBATCH --time=72:00:00             # 최대 시간
#SBATCH --output=logs/%x_%j.out     # 로그 저장 경로
#SBATCH --error=logs/%x_%j.err      # 에러 로그 저장 경로
#SBATCH --exclude=n68
#
# MaNo Attack Comparison: 3 architectures x 3 datasets, parallelized across 3 GPUs.
#
# Usage:
#   bash bash/run_attack.sh
#
# Override attack parameters via environment variables:
#   ATTACK_MODE=pgd ATTACK_STEPS=10 bash bash/run_attack.sh
#   ATTACK_MODE=fgsm bash bash/run_attack.sh
#   ATTACK_MODE=analytical ATTACK_TAU=0.01 bash bash/run_attack.sh  (default, fastest)

DATA_ROOT=./datasets
RESULTS_DIR="${RESULTS_DIR:-results_attack}"

# Attack mode: analytical (default, zero-cost) | fgsm (1-step) | pgd (iterative)
ATTACK_MODE="${ATTACK_MODE:-analytical}"
...
WANDB_GROUP="${WANDB_GROUP:-attack_comparison-pgd}"
WANDB_TAGS="${WANDB_TAGS:-attack,${ATTACK_MODE}}"
...
if [ "$ATTACK_MODE" = "analytical" ]; then
    echo "  tau=${ATTACK_TAU}"
else
    echo "  eps=${ATTACK_EPS}, alpha=${ATTACK_ALPHA}, steps=${ATTACK_STEPS}, gamma=${ATTACK_GAMMA}"
fi
...
# ── Build attack suffix (must match main_attack.py naming) ──────
if [ "$ATTACK_MODE" = "analytical" ]; then
    ATK_SUFFIX="analytical_tau${ATTACK_TAU}"
elif [ "$ATTACK_MODE" = "fgsm" ]; then
    ATK_SUFFIX="fgsm_eps$(printf '%.4f' ${ATTACK_EPS})"
else
    ATK_SUFFIX="pgd_eps$(printf '%.4f' ${ATTACK_EPS})_alpha$(printf '%.4f' ${ATTACK_ALPHA})_steps${ATTACK_STEPS}_gamma${ATTACK_GAMMA}"
fi
